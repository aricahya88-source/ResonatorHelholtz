...
export function MassSpringVibration({ result, waterFillPercent, isBlowing }: MassSpringVibrationProps) {
  ...
  useFrame(({ clock }) => {
    ...
    if (springRef.current) {
      springRef.current.position.y = geom.airCenterY;
      springRef.current.scale.set(
  ...
  return (
    <group visible renderOrder={14}>
      ...
      <line position={[0, geom.airCenterY, 0]} ref={springRef} renderOrder={14}>
        <primitive object={geometry} attach="geometry" />
        <lineBasicMaterial
          color="#fef08a"
          transparent
          opacity={isBlowing ? 0.86 : 0.34}
          depthTest={false}
          depthWrite={false}
        />
      </line>
